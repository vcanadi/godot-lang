class_name CliMsg extends object
  ...
  var field_GET_LISTS_glList: Array[Array[float]]
  var field_GET_LISTS_glList2: Array[Action]
  var field_GET_LISTS_glList3: Array[Array[Array[Action]]]
  ...
  static func get_lists(var field_GET_LISTS_glList: Array[Array[float]], var field_GET_LISTS_glList2: Array[Action], var field_GET_LISTS_glList3: Array[Array[Array[Action]]]) -> CliMsg:
    var ret: CliMsg = CliMsg.new()
    ...
    ret.field_GET_LISTS_glList3 = field_GET_LISTS_glList3
    return ret
  ...
  static func desFromArr(var arr: Array[Variant]) -> CliMsg:
    ...
    match arr[0]:
      ...
      Con.GET_LISTS:
        ret.field_GET_LISTS_glList.assign(arr.map(func(x): x))
        ret.field_GET_LISTS_glList2.assign(arr.map(desFromArr))
        ret.field_GET_LISTS_glList3.assign(arr.map(func(x): x.map(func(x): x.map(desFromArr))))
      Con.GET_MAP:  ret.field_GET_MAP_gmMap = arr[1]
  ...
  static func serToArr(var this: CliMsg) -> Array[Variant]:
    match this.con:
      ...
      Con.GET_LISTS:  return  [ Con.GET_LISTS, field_GET_LISTS_glList.map(func(x): x), field_GET_LISTS_glList2.map(serToArr), field_GET_LISTS_glList3.map(func(x): x.map(func(x): x.map(serToArr))) ]
      Con.GET_MAP:  return  [ Con.GET_MAP, field_GET_MAP_gmMap ]
